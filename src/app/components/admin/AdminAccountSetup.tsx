...
import { checkAdminAccountExists, setupAdminAccount } from "../../services/supabase/admin";
  ...
  useEffect(() => {
    const checkAdminAccount = async () => {
      try {
        ...
        if (import.meta.env.DEV) console.log("🔍 Checking if admin account exists...");
        ...
        try {
          ...
          if (import.meta.env.DEV) console.log("📊 Admin check result from server:", data);
          setAdminExists(!!data.exists);
        } catch (fetchError) {
          if (import.meta.env.DEV) console.log("⚠️ Network error checking admin:", fetchError);
          // On any error, assume admin doesn't exist and show setup button
          setAdminExists(false);
        }
      } catch (error) {
        if (import.meta.env.DEV) console.log("ℹ️ Error checking admin account:", error);
        setAdminExists(false);
      } finally {
        setCheckingAdmin(false);
      }
  ...
  const createAdminAccount = async () => {
    ...
    try {
      ...
      if (!signInError && signInData.session) {
        if (import.meta.env.DEV) console.log("✅ Admin already exists - signed in successfully!");
        setResult({
          type: "success",
      ...
      if (import.meta.env.DEV) console.log("✅ Admin account created:", data);

      // Now sign in with the new credentials
      if (import.meta.env.DEV) console.log("📝 Signing in...");
      ...
      if (import.meta.env.DEV) console.log("✅ Signed in successfully");
      ...
      if (error.name === "TypeError" && error.message.includes("fetch")) {
        setResult({
          type: "error",
          message:
            "⚠️ Edge Function not responding. Please ensure the Edge Function is deployed in Supabase Dashboard > Edge Functions.",
        });
  ...
  return (
    <div className="fixed bottom-4 left-4 bg-purple-600 text-white p-4 rounded-lg shadow-lg z-50 max-w-sm">
      <div className="flex items-start justify-between mb-2">
        ...
        <button onClick={() => setShow(false)} className="text-white hover:text-gray-200 ml-2">
          ✕
        </button>
      </div>
      ...
      <p className="text-sm">
        To create it, use the <strong>Sign Up</strong> button with email{" "}
        <strong>{ADMIN_EMAIL}</strong> and it will automatically be recognized as the admin account.
      </p>
    </div>
  );
}
