...
export default function InsurerPartnershipPage({ onBackToHome }: InsurerPartnershipPageProps) {
  ...
  return (
    ...
    >
      {/* Atmospheric depth — cool-blue calm-workspace tier matching About sub-page */}
      <div className={`bd-bloom-atmosphere ${isVisible ? "is-visible" : "is-hidden"}`} aria-hidden>
        {isLight ? (
          <>
            ...
            <div className="absolute -top-10 left-[12%] w-80 h-80 bg-blue-300/[0.16] rounded-full blur-[120px]" />
            <div className="absolute bottom-10 right-[8%] w-72 h-72 bg-sky-300/[0.14] rounded-full blur-[110px]" />
          </>
        ) : (
          <>
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_70%_45%_at_50%_0%,rgba(59,130,246,0.16),transparent_70%)]" />
            <div className="absolute -top-10 left-[12%] w-80 h-80 bg-blue-500/[0.14] rounded-full blur-[120px]" />
            <div className="absolute bottom-10 right-[8%] w-72 h-72 bg-indigo-500/[0.12] rounded-full blur-[110px]" />
          </>
        )}
      </div>
      ...
      <div className="max-w-5xl mx-auto relative" ref={heroRef}>
        ...
        >
          ...
          {/* Editorial flanking accent on H1 — cool-blue family for sub-page register */}
        ...
        <section
          className={`bd-glass-card bd-glass-card--landing p-6 md:p-8 transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
        ...
        >
          ...
          <a
            ...
            className={`font-semibold transition-colors ${isLight ? "text-blue-700 hover:text-blue-800" : "text-blue-300 hover:text-blue-200"}`}
          >
            bidondent@gmail.com
          </a>
          <div className="mt-5">
