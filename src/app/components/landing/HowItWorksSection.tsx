...
export default function HowItWorksSection({
  ...
  return (
    <section
      ...
      style={{
        background: isLightAppearance
          ? "linear-gradient(175deg, #091c38 0%, #0c2a46 45%, #0a2040 100%)"
    ...
    >
      {/* Edge blend */}
      <div
        className={`absolute -top-px left-0 right-0 h-px bg-gradient-to-r from-transparent ${isLightAppearance ? "via-blue-400/25" : "via-blue-400/30"} to-transparent`}
      />
      {/* Atmospheric depth */}
      {isLightAppearance ? (
        <>
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_60%_50%_at_70%_-10%,rgba(99,102,241,0.10),transparent_60%)]" />
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_50%_50%_at_20%_80%,rgba(59,130,246,0.08),transparent_55%)]" />
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_55%_35%_at_40%_90%,rgba(255,191,105,0.03),transparent_50%)]" />
          <div className="absolute top-10 left-[20%] w-48 h-48 bg-blue-400/[0.06] rounded-full blur-[100px]" />
        </>
      ) : (
        <>
      ...
      >
        <div
          className={`w-8 h-8 rounded-lg flex items-center justify-center ${isLightAppearance ? "bg-blue-500/12 border border-blue-400/18" : "bg-blue-500/15 border border-blue-400/20"}`}
          style={{
            boxShadow: isLightAppearance
        ...
        >
          <Wrench
            className={`w-3.5 h-3.5 ${isLightAppearance ? "text-blue-400/55" : "text-blue-400/50"}`}
          />
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 max-w-7xl relative">
        ...
        <div className="grid md:grid-cols-3 gap-4 md:gap-6 relative">
          ...
          {steps.map((step, index) => (
            <div
              key={step.number}
              className={`rounded-2xl border p-5 sm:p-6 hover:shadow-xl transition-all duration-500 group relative ${isLightAppearance ? "border-blue-300/20 hover:border-blue-300/35" : "border-blue-300/22 hover:border-blue-400/38"} ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
              style={{
                transitionDelay: `${0.3 + index * 0.15}s`,
                background: isLightAppearance
                  ? "linear-gradient(180deg, rgba(15, 30, 60, 0.28) 0%, rgba(10, 18, 40, 0.72) 100%)"
