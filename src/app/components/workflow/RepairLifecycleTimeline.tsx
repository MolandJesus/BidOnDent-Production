...
export default function RepairLifecycleTimeline({
  ...
  return (
    <section className={`bd-glass-card p-4 md:p-5${isLight ? " bd-light-surface" : ""}`}>
      <div className="mb-4">
        <h3 className={`text-xl font-semibold ${isLight ? "text-slate-900" : "text-slate-100"}`}>
          {title}
        </h3>
        {subtitle && (
          <p className={`text-sm mt-1 ${isLight ? "text-slate-600" : "text-slate-300/80"}`}>
            {subtitle}
          </p>
        )}
      </div>
            ...
            <div key={step.id} className="flex gap-3">
              <div className="flex flex-col items-center">
                ...
                {index < steps.length - 1 && (
                  <div
                    className={`w-px h-8 mt-1 ${isLight ? "bg-slate-200" : "bg-white/[0.10]"}`}
                  />
                )}
              </div>
