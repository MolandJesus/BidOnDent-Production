  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bd-glass-floating rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className={`sticky top-0 backdrop-blur-sm border-b px-6 py-4 rounded-t-2xl z-10 ${isLight ? "bg-white/95 border-slate-200/60" : "bg-slate-900/80 border-white/[0.08]"}`}>
          <div className="flex justify-between items-center">
            <h2 className={`font-bold ${isLight ? "text-slate-900" : "text-slate-100"}`}>Edit Profile</h2>
            <button
        ...
        <div className="px-4 sm:px-6 py-5 sm:py-6 space-y-6">
          ...
          <div className="flex flex-col items-center">
            ...
            <p className={`text-xs mt-2 ${isLight ? "text-slate-500" : "text-slate-400"}`}>Click camera to change photo</p>
          </div>
          ...
          <div className="space-y-4">
            {/* Name Field */}
            <div>
              <label htmlFor="edit-name" className={`block text-sm font-medium mb-1.5 ${isLight ? "text-slate-700" : "text-slate-300"}`}>
                Full Name
              </label>
              <div className="relative">
        ...
        {/* Footer Actions */}
        <div className={`sticky bottom-0 border-t px-6 py-4 rounded-b-2xl flex gap-3 z-10 ${isLight ? "bg-white/95 border-slate-200/60" : "bg-white/[0.04] border-white/[0.10]"}`}>
          <button
            type="button"
