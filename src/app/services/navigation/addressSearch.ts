...
function buildPrimaryLabel(result: NominatimSearchResult) {
  ...
  const locality =
    result.address?.city || result.address?.town || result.address?.village || result.address?.county;
...
export async function searchNavigationAddresses(
  ...
  const results = data
    .map((result) => ({
      ...
      primaryLabel: buildPrimaryLabel(result) || result.display_name.split(",")[0] || "Address result",
