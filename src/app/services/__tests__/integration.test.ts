...
export async function testRealtimeService() {
  ...
  console.assert(healthAfterSub.activeSubscriptions === 1, "✅ Subscription active");
  ...
  console.assert(healthAfterUnsub.activeSubscriptions === 0, "✅ Subscription cleaned up");
...
export async function testPerformanceOptimizer() {
  ...
  console.assert(data1.timestamp === data2.timestamp, "✅ Cached data returned (same timestamp)");
  ...
  console.assert(data3.timestamp !== data2.timestamp, "✅ New data fetched (different timestamp)");
...
export async function testStorageService() {
  ...
  const publicUrl = storageService.getPublicUrl("bidondent-profiles", "test/photo.jpg");
...
export async function testIntegration() {
  ...
  const unsubscribe = realtimeBidService.subscribeToReportBids(reportId, async (newBid) => {
    bidCount++;
    console.log("✅ Bid received:", newBid);

    // Cache the bid
    await performanceOptimizer.cachedQuery(`bid-${newBid.id}`, async () => newBid, 60000);
...
export async function testErrorHandling() {
  ...
  const unsub = realtimeBidService.subscribeToReportBids("error-test-report", (bid) => {
...
export const manualTests = {
  ...
  testStorage() {
    ...
    const publicUrl = storageService.getPublicUrl("bidondent-profiles", "test.jpg");
