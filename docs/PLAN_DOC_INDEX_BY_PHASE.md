...
| Doc                                             | Tier | Purpose                                                                                                                                                                                         | Notes                                                                                                                                                                                                                    |
| ----------------------------------------------- | ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `OPS_PHASE_7_PRE_EXECUTION_AUDIT_2026-05-04.md` | OPS  | Read-only audit of Phase 7 surfaces (bids, report wizard map step, shop directory, insurer screens) against `LAW_LAYERED_ARCHITECTURE.md` + `MOLANDJESUS`. Mirrors Phase 6 + 6.5 audit pattern. | **SHIPPED 2026-05-04.** Findings: 0 defensive (1 P6 already-tracked KI-111). 0 hard-limit violations. KI-108–112 inventories all verified. Phase 7 awaiting owner re-scope per Path A/B/C in audit §8.                   |
| `OPS_BIDS_REPORT_SHOP_INSURER_MAP_LOG.md`       | OPS  | Per-commit execution log for the remaining-map-surface redesign. Includes shop/ sub-folder split decision if shop dir grows past hard limit during this phase.                                  | **CONDITIONAL** — written only if owner picks Path A and the `shop/` sub-folder split produces fix commits. Otherwise NOT WRITTEN; the audit doc is the durable record (same outcome as Phase 6 + 6.5 close-only paths). |
