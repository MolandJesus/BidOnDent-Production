...
- **Third half — consumer gap (audit AI Pass 12 finding):** **OPEN.** Pass 179's `flagImplausibleRoute` annotations (`isImplausible`, `implausibleReasons` on `RouteOption`) are **set on the data but NOT consumed by any UI component.** `grep -rn "isImplausible\|implausibleReasons" src/app` returns only the type definition and the producer; zero render-site consumers. Pass 179 fires a dev console.warn but no component dims, hides, or labels implausible alternatives in the route preview chip row or the live navigation HUD. The data-side flag is non-load-bearing until consumers wire it. Pass 13+ candidate: gate `ShopDirectoryRoutePreviewCard` chip rendering on `!route.isImplausible` (hide implausible alternatives entirely) or render with a "Verify route" warn pill (surface the reason without hiding the data).
...
- **Status:** **PARTIAL — sanity flag SHIPPED 2026-05-08 (Pass 179); display bug STAGED 2026-05-08 (Pass 13 audit AI, awaiting commit); consumer gap + upstream coordinate fix OPEN.**
