...
- **Remaining work:** F.3 (passive listeners audit), F.4 (defer/idle GPS speed-limit init off the landing render path), F.5 (provider-side investigation — out of pre-launch scope). Each scoped as a separate pass.
- **Status:** PARTIAL RESOLUTION (2026-05-07, Passes 49 + 85) — F.1 + F.2 shipped; F.3-F.5 still open. P4 (polish; no functional regression).
