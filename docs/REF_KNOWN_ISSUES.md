...
- **Sub-pass 3 SHIPPED Pass 196:** `<MapTilePicker>` mount in the same file is now gated `!isGuidanceActive && !suppressTilePicker && mapLoaded && !mapLoadFailed` and wrapped in the same `<div className="map-ui-enter">` cross-fade wrapper. Same pattern as sub-pass 1 — eliminates the third faded overlay (Map/Night/Satellite tile toggle) from the pre-hydrated load window. Build PASS (4.23s). Remaining: sub-pass 2 (top-chrome `MapSurfaceHeaderBadges` host gating, or owner confirmation that header badges are intentionally render-eager).
- **Severity:** **P2-LOADING.**
- **Status:** **PARTIALLY RESOLVED (Pass 194 sub-pass 1 + Pass 196 sub-pass 3 of 3).** Both bottom-anchored overlays + tile picker now gated. Remaining: top-chrome header badges + owner re-test + visual baseline re-capture. Hold open until owner re-tests and confirms.
