...
- **Status:** RESOLVED 2026-05-03. Multi-pass sweep across the dashboard, landing, and map control surface families:
  ...
  - **Remaining deferred scope (KI-066b — split):** `src/styles/theme.css` still carries ~108 forbidden-register occurrences across deep premium surface layers (bd-glass-_ shadow stacks, bd-dashboard-section--accent-_ families, bd-dashboard-section--interactive sheen sweep, hero panel atmospherics, panel/section border tokens at 2504/2510/2525, sidebar accent at 2607). Each layer composes a different part of the shipped premium register, so a single bulk find/replace risks dimming the gold character (220→196 is bronzier and less honey). Tracked as the next dedicated pass with isolated screenshot verification per surface family.
