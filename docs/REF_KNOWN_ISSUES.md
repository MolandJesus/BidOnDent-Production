...
- **Severity:** **P1-LAYOUT.**
- **Status:** **HOLD — PINPOINTED 2026-05-08 (audit AI Pass 11).** Pass 10 left this as "host file unidentified within budget." Pass 11 code-level grep on the unique audit-evidence strings ("Top pick", "in-repair") locates the host:
  - **Legend host:** [`src/app/components/shop/MapPaneLegendPanel.tsx`](../src/app/components/shop/MapPaneLegendPanel.tsx) (206 lines). Renders the dual legend (semantic Origin/Selected/Top pick/Reports/Saved/Routes at lines 90–200, plus status filter chips All/Pending/Approved/In Repair/Resolved/Done at lines 167–198).
  - **Mount site:** [`src/app/components/shop/ShopDirectoryMapPaneOverlays.tsx:294`](../src/app/components/shop/ShopDirectoryMapPaneOverlays.tsx#L294) — inside `MapPaneBottomOverlay` (the wrapper that also renders the selected-shop card). The wrapper at line 135 sets `pointer-events-none absolute inset-x-0 bottom-0 z-[500]`. Selected-shop card and legend are flex-wrap siblings inside the same container.
  ...
  - **Reframe of audit measurement:** the audit's "ROUTE box at y:655–865 (z:510)" is the **selected-shop card** with route-source badge ("Estimated route" / "Live route"), distance label, and ETA — not a separate routing panel. The 129px overlap arises because shop card and legend are flex-wrap siblings inside the same absolute-bottom container; on narrower viewports the wrap stacking pushes the legend up under the card.
  - **Existing precedent for the fix (Pass 78 / KI-140):** `MapPaneLegendPanel` already supports a `density="compact"` mode that collapses to a single tap-to-expand "Legend" pill (lines 32–67). That fix landed for mobile fullscreen but NOT for default desktop density.
  - **Recommended fix (NOT IMPLEMENTED THIS PASS per master-builder dispatch — pinpoint + recommend only):** extend the Pass 78 / KI-140 collapse-to-pill pattern to default density. Either (a) flip `useState(!isCompactDensity)` → `useState(false)` so default density is also collapsed-by-default, or (b) gate on viewport height (e.g. `< 900px → start collapsed`). Master builder Pass 180 §7.3 already approved option (a) collapsed-with-toggle for KI-166. Same change resolves both KI-164 and KI-166.
  - **Co-worker AI alignment (cited as suggestion-only, not LAW):** the Pass 11 owner-decision brief reaffirms option (a) over hover-tooltip on accessibility + touch grounds. Default-collapsed-state recommendation: do NOT remember last state; always start collapsed on mount, otherwise first-time visitors land on the 20%-viewport bug.
- **Companion:** master builder dispatch authorizes DOM-inspection on `/dashboard` Smart Shop Map fullscreen for next-pass implementation; the recommendation above can be applied as a 1–3 line change in `MapPaneLegendPanel.tsx` once that gate clears. The structural slot relocation in Step C.2 of [`PLAN_MAP_UNIFICATION_2026-05-08.md`](PLAN_MAP_UNIFICATION_2026-05-08.md) is independent and still applies.
...
- **Severity:** **P2-LAYOUT.**
- **Status:** **HOLD — REFINED 2026-05-08 (audit AI Pass 11).** Pass 10 traced the source string to [`shopMapExperienceHelpers.ts:224`](../src/app/services/intelligence/shopMapExperienceHelpers.ts#L224) and hypothesized `ImmersiveMapTopBar` as the render site. Pass 11 falsifies both:
