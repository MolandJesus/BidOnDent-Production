...
- **Status:** **PARTIAL — Pass 64 + Pass 67 + Pass 72 (2026-05-07).** Items #1, #2, #3, #4, #5, #6 RESOLVED. (#1) `BidsSummaryHeader` mobile + desktop now read "1 offer" when `bidCount === 1`. (#2) New shared helper [`src/app/utils/formatVehicleLabel.ts`](../src/app/utils/formatVehicleLabel.ts) suppresses duplicated leading make ("Mazda Mazda6" → "Mazda6"); applied in `ReportDetailScreen`, `ReportsListScreen`, `insurerClaimsUtils`. (#3) Smoke Test Checklist row already gated behind `import.meta.env.DEV` in `AccountMenu.tsx` — verified, no change needed. (#4) `SettingsModal` now commits appearance change on toggle (matches "save immediately" helper text); footer button renamed to "Done"; unused `Save` lucide import removed. (#5) New `getNavigationVoicePersonaLabel` helper in [`voiceGuidance.ts`](../src/app/services/navigation/voiceGuidance.ts) maps the persona key (`british-smooth`) to a stable display label ("British (smooth)"). [`useShopDirectoryNavigation.ts`](../src/app/hooks/useShopDirectoryNavigation.ts) now feeds `preferredVoiceLabel` from this helper instead of the OS-resolved `voice.name` (which varied by browser). The two consuming surfaces (`NavigationVoiceControlsSheet`, `PlannerVoiceGpsSettings`) now display the persona consistently. (#6) Added `overflow-hidden` to the "Browse all shops & AI matching" CTA in `CustomerMapWidget` so the gradient is clipped by `rounded-xl`. Items #7, #8, #9, #10 still **OPEN** (need DOM/design decisions).
