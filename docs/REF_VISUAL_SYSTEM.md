...
**Pass 22 (2026-05-06) — background-position drift attempt evaluated and declined.** Brief authorized a `background-position`-based drift on `.bd-bloom-atmosphere::before` + `::after` (proposed periods 26s + 42s, phase offset −5s) on the rationale that animating `background-position` instead of `transform` would sidestep the double-stack with the scroll-entry `transform: scale(...)`. On inspection the host element has **no own gradient** — it is an empty wrapper around child atmosphere layers (the wrapped section's existing lamps/orbs are the visible content). Pseudo-elements would have to introduce a brand-new gradient to drift, which means adding new visual content (a new lamp/wash on top of whatever the section already paints) rather than animating existing atmosphere. That changes the surface's visual identity, not just its motion. Decision: ship the no-op. The no-drift exception stands. Future revisit would require either (a) owner-named visual identity change to the wrapper itself, or (b) extending drift down into the wrapped child surfaces individually (which is what the existing `bd-landing-section-toplamp` / `bd-landing-section-bottomwash` ledger already does for the landing case).
