...
| 57   | Dual off-route paths consolidation (F3)                   | Pending                 | —             | Pass 53 §D documented duplication. Goal: explicit guard preventing OSRM double-fire when reroute lifecycle is `pending`/`cooldown`. ~30 LOC, ≤2 files.                                  |
