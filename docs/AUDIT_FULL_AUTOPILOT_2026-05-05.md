...
| A3 | 36 ungated `console.log` in production paths (not behind `import.meta.env.DEV`). Notable: `useUserData.ts` 8x `[DEBUG]` logs, `useUserDataLoader.ts` 4x logs, `RealtimeBidService.ts` startup logs, admin component logs, `StorageService.ts` provider init. These ship to production console — noise on every user session, plus a security/PII concern (e.g. `useAppEffects.ts:52` logs Clerk user info). | P2 | FIX THIS PASS — gate the noisiest 6-10 with `if (import.meta.env.DEV)` |
