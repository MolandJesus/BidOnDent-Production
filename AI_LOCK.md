...
**Last commit:** Pass 259 — PMS architecture options matrix (`docs/PLAN_PMS_ARCHITECTURE_OPTIONS_2026-05-09.md`). 12-axis × 6-option scorecard evaluating shell singleton (O1) / hidden-root persistence (O2) / preserved subtree (O3) / portal transfer (O4) / detached renderer (O5) / hybrid persistence (O6) against route authority, React correctness, WebGL lifecycle stability, memory lifetime, reduced-motion continuity, performance gates G1-G4, rollback complexity, instrumentation compatibility, migration sequencing, semantic-risk surface, app-shell location compatibility, and PMS-impacted controller bridge cost. Recommends **O6 (hybrid persistence) using O1 sub-pattern for E1, hosted at Pass 258 §15 candidate (d) — new `MapSessionProvider` between Clerk and `App`**. Recommendation explicitly excludes E3 from PMS to preserve Tier B branch semantics by construction. Includes 4-item dissent log, 8-item forward-constraint list for Pass 260, 10-item risk register with mitigations. PMS planning corpus state: Passes 258 + 261 + 259 landed; Pass 260 (execution sequencing) authorized for autopilot continuation. Suite: 927/927 across 91 files. autoFit/callerBoundsExplicit/sub-pass C/ShopMapWidget UNTOUCHED. No runtime semantic changes.
